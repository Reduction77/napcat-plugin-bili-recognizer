...
test('页面先加载详情组件，再加载 app，支持带前缀的 NapCat 地址',async()=>{
 ...
 listeners.DOMContentLoaded();const helper=appended.at(-1);assert.equal(helper.src,'/prefix/plugin/bili/files/static/download-details.js?v=1.6.0');assert(!appended.some(e=>e.src?.includes('/app.js')));helper.onload();assert.match(appended.at(-1).src,/app.js\?v=1.6.0$/);
});
