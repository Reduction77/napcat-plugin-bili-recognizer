...
test('注册专属页面，所有业务接口走 NapCat 鉴权路由',async t=>{const {call,pages}=await setup(t);assert.equal(pages[0].path,'dashboard');const s=await call('GET','/status');assert.equal(s.data.version,'1.6.1');assert(!JSON.stringify(s).includes('secret-session'));});
