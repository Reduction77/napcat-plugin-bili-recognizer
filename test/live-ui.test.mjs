...
test('直播设置提交到鉴权 API，保存时保留另一个未保存表单',async()=>{
 ...
 assert.equal(x.requests[0].url,'/api/Plugin/ext/bili/live/settings');assert.deepEqual(x.requests[0].body,{revision:2,enabled:true,intervalSeconds:90,scheduleEnabled:true,windows:[{startTime:'22:00',endTime:'02:00'}],continueUntilEnd:true});
...
test('手动开始、停止、恢复按钮调用对应控制接口',async()=>{
 for(const action of ['start','stop','auto']){const x=setup();const b={id:'live-detect-'+action,dataset:{},disabled:false,classList:{contains:()=>false}};
  ...
  assert.equal(x.requests.length,1);assert.equal(x.requests[0].url,'/api/Plugin/ext/bili/live/control');assert.deepEqual(x.requests[0].body,{revision:2,action});
 }
});
...
test('浏览器禁止原生确认框时，开播和下播仍可经页内确认提交；取消不发送',async()=>{
 for(const kind of ['start','end']){
  ...
  await click(x,'',{uid:'123',kind},'live-manual-notify');assert.equal(x.requests.length,0);
  ...
  await click(x,'live-manual-cancel');assert.equal(x.requests.length,0);assert.equal(x.node('#live-manual-panel').hidden,true);
  await click(x,'',{uid:'123',kind},'live-manual-notify');await click(x,'live-manual-send');
  assert.equal(x.requests.length,1);assert.equal(x.requests[0].url,'/api/Plugin/ext/bili/live/notify');assert.deepEqual(x.requests[0].body,{revision:2,uid:'123',kind});assert.match(x.node('#live-action-status').textContent,/已排队/);
 }
});
test('单群补发确认显示原群并传递原记录；刷新保留确认内容和对应版本',async()=>{
 ...
 await click(x,'live-manual-send');assert.deepEqual(x.requests[0].body,{revision:2,uid:'123',kind:'end',recordId:'record-1'});
});
...
test('未保存表单阻止手动发送时提供持续可见反馈，重复点击不会重复提交',async()=>{
 const x=setup();vm.runInContext('dirty=true',x.context);await click(x,'',{uid:'123',kind:'start'},'live-manual-notify');assert.equal(x.requests.length,0);assert.equal(x.node('#live-action-status').hidden,false);assert.match(x.node('#live-action-status').textContent,/未保存/);
...
test('增删时段保留尚未保存的时间，保存一次提交全部时段',async()=>{
 ...
 assert.deepEqual(x.requests[0].body.windows,[{startTime:'12:00',endTime:'14:00'},{startTime:'22:00',endTime:'02:00'}]);
