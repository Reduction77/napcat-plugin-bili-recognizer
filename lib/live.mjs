...
const initial=()=>({version:1,revision:0,enabled:false,intervalSeconds:60,...scheduleDefaults,manualTargets:[],subscriptions:[],states:{},rooms:{},manualNotices:{},records:[]});
...
export async function fetchLive(client,uid){
 ...
 return {uid,roomId:String(r.room_id),name:clean(r.uname,80)||`UID ${uid}`,title:clean(r.title),status:r.live_status,startAt:Number.isFinite(Number(r.live_time))&&Number(r.live_time)>0?Number(r.live_time)*1000:0};
}
...
export class LiveMonitor{
 ...
 async manualNotify(b){
  ...
  try{
   await this.transact(d=>{
    ...
    let targets=sub.groups,text=liveMessage(sub,room,b.kind,this.now());
    if(b.recordId!==undefined){
     ...
     targets=[r.group];if(r.text)text=r.text;
    }
    const now=this.now(),batchId=randomUUID();
    const rows=targets.map(group=>({id:randomUUID(),batchId,time:now,uid:b.uid,group,kind:b.kind,source:'manual',text,status:'queued',message:''}));
    ...
    d.revision++;batch={rows,sub:structuredClone(sub)};
   });
 ...
 async sendManualBatch({rows,sub}){
  ...
  for(const row of rows){
   ...
   try{await this.send(row.group,row.text);this.log('info',`[LIVE][MANUAL] 发送成功 UID=${sub.uid} 群=${row.group} 类型=${row.kind}`);}catch{status='uncertain';message='发送失败或回执超时，请检查群消息后按需补发。';this.log('warn',`[LIVE][MANUAL] 手动直播通知发送异常（UID ${sub.uid}，群 ${row.group}）。`);}
   if(this.closed)return;
 ...
 async poll(){
  ...
  for(const sub of this.data.subscriptions.filter(s=>s.enabled)){
   ...
   for(const group of notice.targets){
    if(!valid())return;
    const row={id:randomUUID(),time:notice.time,uid:sub.uid,group,kind:notice.kind,source:'auto',text:liveMessage(sub,notice.state,notice.kind,notice.time),status:'attempting',message:''};
    ...
    let status='sent',message='';
    try{await this.send(group,liveMessage(sub,notice.state,notice.kind,notice.time));}
