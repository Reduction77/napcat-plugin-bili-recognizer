...
export class LiveMonitor{
 ...
 async compose(sub,state,kind,now){
  const config=this.getState().config||{};
  const card=liveCardData(sub,state,kind,now,{persona:config.liveCardPersona,avatarPosition:config.liveCardAvatarPosition});
 ...
 async manualNotify(b){
  ...
  try{
   await this.transact(d=>{
    ...
    const rows=targets.map(group=>({id:randomUUID(),batchId,time:now,uid:b.uid,group,kind:b.kind,source:'manual',text,card,reply,status:'queued',message:''}));
    ...
    d.revision++;batch={rows,sub:structuredClone(sub),text,reply,card,kind:b.kind,uid:b.uid,state:structuredClone(state)};
   });
  ...
  if(!b.recordId||!batch.card){
   const notice=await this.compose(batch.sub,batch.state||{},b.kind,this.now());
   if(notice.card)batch.card=notice.card;
   batch.reply=notice.reply||batch.reply;batch.text=notice.text;batch.cardAllowed=notice.cardAllowed;
  }
 ...
 async poll(){
  ...
  for(const sub of this.data.subscriptions.filter(s=>s.enabled)){
   ...
   for(const group of notice.targets){
    if(!valid())return;
    const row={id:randomUUID(),time:notice.time,uid:sub.uid,group,kind:notice.kind,source:'auto',text:composed.text,card:composed.card||'',reply:composed.reply||'',status:'attempting',message:''};
