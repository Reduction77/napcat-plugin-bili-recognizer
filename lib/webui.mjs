...
export function registerWebUI(ctx,getState,updateConfig,log){
 ...
 route('get','/live/card',s=>({enabled:!!s.config.liveCard,configured:!!s.config.liveCardRenderUrl,renderUrl:s.config.liveCardRenderUrl,mode:s.config.liveCardRenderMode,persona:s.config.liveCardPersona,personaLabel:personaLabel(s.config.liveCardPersona),personas:personas.map(key=>({key,label:personaLabel(key)})),theme:s.config.liveCardTheme,themes,decor:s.config.liveCardDecor,timeoutSeconds:s.config.liveCardRenderTimeoutSeconds,status:s.live.card.status_(),hint:s.config.liveCardRenderUrl?'':renderHint('OFF')}));
 route('post','/live/card/preview',async(s,req)=>{
  ...
  const card=liveCardData(sub,state,kind,now,{persona});
