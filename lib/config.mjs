...
export const schema=[
 ...
 bool('bareIds','识别单独发送的 BV / AV / CV 编号'),bool('showCover','发送封面图片','图片发送失败时尝试改发纯文字。'),bool('showStats','显示播放、点赞等统计'),bool('showDescription','显示简介'),
 ...
 {key:'liveCardAvatarPosition',label:'头像取景位置',type:'select',default:'center 35%',options:[{label:'偏上（默认，适合头顶贴边）',value:'center 35%'},{label:'居中',value:'center center'},{label:'偏下（适合人物偏下）',value:'center 65%'},{label:'取上部三分之一',value:'center 20%'},{label:'取下部三分之一',value:'center 80%'}],description:'头像是圆形显示，方图的四个角会被切掉。人物头顶被切或偏下时，换一个取景位置即可。'},
...
export function normalize(raw={}) {
 ...
 if(!/^(center|top|bottom|left|right|\d{1,3}%)(\s+(center|top|bottom|left|right|\d{1,3}%))?$/.test(c.liveCardAvatarPosition))throw new Error('头像取景位置无效');
