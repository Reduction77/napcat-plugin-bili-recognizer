...
// 手动通知允许用本地上传/粘贴的图片：只接受图片类型的 data URI，并限制体积。
export const DATA_IMAGE_MAX_CHARS=3*1024*1024;
export function dataImage(value){
 const text=String(value??'').trim();
 if(!text.startsWith('data:image/'))return '';
 if(text.length>DATA_IMAGE_MAX_CHARS)return '';
...
export function liveCardData(sub,state,kind,now,{persona='amis',avatarPosition='center 35%'}={}){
 ...
 return {persona:style,name,kind,title:titleText,room,url:room,area,duration,timeText,meta,lines,reply,text,card:style!=='plain',avatarPosition,summary:lines.join(' ')};
}
...
export function renderCardHtml(card,{theme='amis',stickers={}}={}){
 ...
 const avatarPos=String(card.avatarPosition||'center 35%');
 const avatar=card.avatar?`<img class="avatar-img" style="object-position:${escapeAttr(avatarPos)}" src="${escapeAttr(card.avatar)}" alt="">`:'<div class="avatar-img avatar-empty">爱</div>';
...
export const THEME_CSS={
 amis:`
:root{--font:'Noto Sans CJK SC','Source Han Sans SC','Microsoft YaHei','PingFang SC','WenQuanYi Zen Hei','Noto Color Emoji','Apple Color Emoji','Segoe UI Emoji',system-ui,sans-serif;--brand:#ff8fc0;--brand-deep:#e75f9c;--ink:#3b2b38;--soft:#8a7a86;--card:#fff;--bg:#fff0f6;--bubble:#ffe6f1;--bubble-ink:#d9427f;}
...
.avatar{box-sizing:border-box;width:96px;height:96px;flex:0 0 96px;border-radius:50%;background:#fff;border:3px solid #fff;box-shadow:0 4px 14px rgba(231,95,156,.28);display:flex;align-items:center;justify-content:center;overflow:hidden;}
.avatar-img{width:100%;height:100%;border-radius:50%;object-fit:cover;object-position:center 35%;display:block;background:var(--brand);}
...
`,
 pink:`
:root{--font:'Noto Sans CJK SC','Source Han Sans SC','Microsoft YaHei','PingFang SC','WenQuanYi Zen Hei','Noto Color Emoji','Apple Color Emoji','Segoe UI Emoji',system-ui,sans-serif;--brand:#fb7299;--brand-deep:#e05c85;--ink:#3b2b38;--soft:#8d8189;--card:#fff;--bg:#fdf3f6;--bubble:#fff2f6;--bubble-ink:#c9416d;}
body{margin:0;background:var(--bg);font-family:var(--font);}
...
.avatar{box-sizing:border-box;width:96px;height:96px;flex:0 0 96px;border-radius:50%;background:#fff;border:3px solid #fff;box-shadow:0 4px 14px rgba(251,114,153,.3);display:flex;align-items:center;justify-content:center;overflow:hidden;}
.avatar-img{width:100%;height:100%;border-radius:50%;object-fit:cover;object-position:center 35%;display:block;background:var(--brand);}
.avatar-empty{display:flex;align-items:center;justify-content:center;color:#fff;font-size:40px;font-weight:700;}
.who{flex:1;min-width:0;padding:6px 0;}
...
`,
 min:`
:root{--font:'Noto Sans CJK SC','Source Han Sans SC','Microsoft YaHei','PingFang SC','WenQuanYi Zen Hei','Noto Color Emoji','Apple Color Emoji','Segoe UI Emoji',system-ui,sans-serif;--brand:#5b6b7c;--brand-deep:#33414f;--ink:#20262c;--soft:#77828c;--card:#fff;--bg:#f4f6f8;--bubble:#eef2f6;--bubble-ink:#33414f;}
body{margin:0;background:var(--bg);font-family:var(--font);}
...
.avatar{box-sizing:border-box;width:86px;height:86px;flex:0 0 86px;border-radius:50%;background:#fff;border:3px solid #fff;box-shadow:0 3px 10px rgba(32,38,44,.14);display:flex;align-items:center;justify-content:center;overflow:hidden;}
.avatar-img{width:100%;height:100%;border-radius:50%;object-fit:cover;object-position:center 35%;display:block;background:var(--brand);}
...
export const AVATAR_THUMB_WIDTH=240;
...
export async function collectCardImages(client,card,{stickerDir,fs,decor='auto'}={}){
 ...
 const fetchDataUri=async(url,{width=0,ratio='' }={})=>{
  // 手动通知里用户自己贴的图片：已经是 data URI，直接内联，不去请求网络。
  const inline=dataImage(url);if(inline)return inline;
  const safe=safeImageUrl(url);if(!safe)return '';
 ...
 images.avatar=await fetchDataUri(card.avatarSource,{width:AVATAR_THUMB_WIDTH});
