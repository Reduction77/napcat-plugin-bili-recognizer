...
const personaReply=({kind,name,titleText,timeText,area,duration})=>{
 if(kind!=='start'){
  const head=duration?`「${name}」陪你播了 ${duration}`:`「${name}」下播咯`;
  const tail=[titleText?`《${titleText}》就先到这里`:''].filter(Boolean).join('，');
  return tail?`${head}，${tail}，辛苦啦！`:`${head}，辛苦啦！`;
 }
 const watch=titleText?`《${titleText}》`:'直播间';
 return `「${name}」正在直播${watch}${area?` · ${area}`:''}${timeText?`，${timeText}已经开播啦`:''}`;
};
const PERSONAS={
 amis:{
  label:'爱弥斯 · 活泼俏皮',
...
export function liveCardData(sub,state,kind,now,{persona='amis'}={}){
 ...
 const reply=personaReply({kind,name,titleText,timeText,area,duration});
...
<div class="card">
 ...
 ${durationRow}${metaRow}
 <div class="row"><span class="k">👉</span><span class="v mono">${escapeHtml(card.url)}</span></div>
...
export async function renderCard(html,config,{fetcher=globalThis.fetch,log=()=>{}}={}){
 ...
 const options={type:'jpeg',quality:92,full_page:true,viewport_width:Number(config.liveCardWidth)||640,viewport_height:1,scale:'device',device_scale_factor_level:'ultra',timeout:Math.round(timeoutMs/1000)-1,animations:'disabled',caret:'hide'};
 let payload;
