...
export function extract(text, bareIds=true) {
  ...
  // 裸编号只放行辨识度高的 BV / AV / CV；ss、ep、au 等两字母前缀在日常聊天中
  // 太容易误撞（如“看 ep3”），这些类型仍可通过完整链接或短链识别。
  if(bareIds) for(const m of remainder.matchAll(/(?<![a-zA-Z0-9_])(?:BV[a-zA-Z0-9]{10}|av\d+|cv\d+)(?![a-zA-Z0-9_])/gi)) {
    const pre=m[0].slice(0,2).toLowerCase();
    const types={bv:'video',av:'video',cv:'article'};
