...
// 静态资源域名白名单匹配：'hdslb.com' 与 '.hdslb.com' 都表示该域名及其子域。
export function hostAllowed(hostname,allowedHosts){
 ...
 return allowedHosts.some(entry=>{
  ...
  if(!rule||!/^[a-z0-9.-]+$/.test(rule))return false;
...
export class BiliClient {
 ...
 // allowedHosts：给非 B站主站的静态资源（图床 *.hdslb.com）开一个受控口子。
 // 只接受精确域名或 ".域名" 后缀，仍然禁止 http、端口、用户名密码，不会变成任意代理。
 async request(url,{method='GET',cookie=false,redirect=false,html=false,raw=false,referer='https://www.bilibili.com/',allowedHosts=[]}={}) {
  const u=new URL(url);
  ...
  if(!cookie&&!safeBiliUrl(url)&&!hostAllowed(u.hostname,allowedHosts))throw new BiliError('HOST','无效跳转域名');
  ...
  try{
   ...
   const r=await this.fetcher(url,{method,redirect:'manual',headers:{'User-Agent':UA,Referer:referer,Accept:html?'text/html,application/xhtml+xml':raw?'image/*,*/*':'application/json, text/plain, */*','Accept-Language':'zh-CN,zh;q=0.9',...(credential?{Cookie:credential}:{})},signal:AbortSignal.any([this.abort.signal,AbortSignal.timeout(this.config.timeoutSeconds*1000)])});
   ...
   if(raw){
    // 二进制资源（封面、头像）：直接交出字节流，不做 JSON 解析。
    return {ok:r.ok,status:r.status,httpStatus:r.status,headers:r.headers,body:r.body,
